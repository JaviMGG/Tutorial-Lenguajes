...
//BigInt
let granEntero =
  BigInt(22222222222222222222222222222222222225649849195194981632225); //numero tan grande que no se puede representar con "number"
...
switch (
  dia //dentro del parentesis, ponemos que variable vamos a "comparar"/determina lo que hacemos
) {
  case 1:
    nombreDia = "dia es 1";
...
miMapa = new Map([
  ["Nombre", "Paco"],
  ["Edad", 24],
  ["Altura", 2.0], //si quisiera añadir más, habria que poner una , y después otro par de elementos en []
]);
console.log(miMapa);
...
const proxyto = {
  ...
  set(target, property, Valor) {
    if (Valor < 0) {
      console.log("valor no puede ser negativo");
    } else {
...
function sum(...params) {
  let res = 0;
  for (let number of params) res += number;
  return res;
}
...
setTimeout(() => {
  console.log("segundo");
}, 2000); //se ejecutará la funcion anónima () despues de 2000/1000 segundos (1 segundo)
console.log("tercero");
